...
class Embedding_Layer2(nn.Module):
    def __init__(self,
        ...
        self.embedding_layer = nn.Sequential(
            nn.Conv3d(feature_nums, feature_nums // 2, 1, 1, 0),  # 첫 번째 Linear Layer (입력: 512 → 출력: 256)
            nn.ReLU(),            # 비선형 활성화 함수
            nn.LayerNorm(normalized_shape=(feature_nums, *input_volume_size)),    # Layer Normalization 적용
            nn.Conv3d(feature_nums, feature_nums, 1, 1, 0),  # 두 번째 Linear Layer (출력: 128)
            nn.LayerNorm(normalized_shape=(feature_nums, *input_volume_size)),    # 최종 Layer Normalization
            nn.ReLU()
        )
